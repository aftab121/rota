   ...
   <input id="shift_currentweekStart" name="shift_currentweekStart" type="hidden" value="<?php echo date('Y-m-d',strtotime($start_date));?>" class="form-control" >
    ...
    <tbody id="tbody">
	 ...
	 <?php 
      ...
      <tr class="tr_even">
        <td colspan="7"><b style="float:left;"><?php echo $User['firstname'].' '.$User['lastname'];?></b><b style="float:right;"><span>Total Hours/ Budget : </span><span style="margin-left:10px;"><?php echo number_format(abs($total_hrs),2).' hrs / $ '.number_format(abs($total_labour_cost),2);?></span></b></td>
      </tr>
      <?php endforeach;?>
	...
	foreach($PositionLists as $Position):?>
      ...
      <tr class="tr_even">
        <td colspan="7"><b style="float:left;"><?php echo $Position['position_name'];?></b><b style="float:right;"><span>Total Hours/ Budget : </span><span style="margin-left:10px;"><?php echo number_format(abs($total_hrs),2).' hrs / $ '.number_format(abs($total_labour_cost),2);?></span></b></td>
      </tr>
      <?php endforeach;?>
	  <?php $show_budget = Session::get('CompanyDetails.budget');
  ...
  <tbody><!--Sales Calculation starts -->
	 ...
	 <?php $show_sales_per_hour = Session::get('CompanyDetails.sales_per_hour');
	  ...
	  <tr class="afterBudget" style="display:none;">
		...
		?>
        <td>
		<span id="salesperHour<?php echo $i;?>"> $ <?php echo @$sales_per_hour[0];?></span><br/>
		<?php } ?>
	    </td>
	  ...
	  <tr class="afterBudget" style="display:none;">
		...
		?>
        <td class="custom-padding">
		 $<input type="number" id="labour_variation<?php echo $i;?>" value="<?php echo  @$labour_variation[0];?>" min="0" name="labour_variation<?php echo $i;?>" value="0" class="form-control labour_variation"/>
		 <br/>
         </td>
        <?php } ?>
      </tr>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body clearfix">
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            <div class="form-group  clearfix">
              <label><?php echo (@$UserLists)?'Position Assignment':'Staff Member assignment';?></label>
              <?php //echo "<pre>";print_r($PositionLists);?>
              <select name="<?php echo (@$UserLists)?'position_id':'assigned_to';?>"  id="<?php echo (@$UserLists)?'edit_position_id_list':'edit_assigned_to_list';?>" class="form-control">
              </select>
            </div>
          </div>
        </div>
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            <div class="form-group  clearfix">
              <label><?php echo (@$UserLists)?'Position Assignment':'Staff Member assignment';?></label>
              <?php //echo "<pre>";print_r($PositionLists);?>
              <select name="<?php echo (@$UserLists)?'position_id':'assigned_to';?>"  id="<?php echo (@$UserLists)?'position_id_list':'assigned_to_list';?>" class="form-control">
              </select>
            </div>
          </div>
        </div>
