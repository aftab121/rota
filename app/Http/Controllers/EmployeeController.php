...
class EmployeeController extends Controller {
	...
	public function employeeSchedule(){
		...
		$inputData =Input::all(); 
		$shifts = Shift::where('shifts.assigned_to','=',$user_id)->where('shifts.status','=',1)->where('shift_date','>=',$start_date)->where('shift_date','<=',$end_date)->leftJoin('locations','locations.location_id', '=', 'shifts.location_id')->leftJoin('positions','positions.id', '=', 'shifts.position_id')->select('locations.location_name','positions.position_name','shifts.*')->get();
